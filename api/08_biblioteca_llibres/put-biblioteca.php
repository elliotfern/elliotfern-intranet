      ...
      if (verificarToken($token)) {
        ...
        // Ruta PUT => "/api/biblioteca/put?llibre"
            ...
            if ($data === null) {
              ...
              if ($hasError == false) {
                  global $conn;
                  $sql = "UPDATE 08_db_biblioteca_llibres SET autor=:autor, titol=:titol, titolEng=:titolEng, any=:any, idGen=:idGen, subGen=:subGen, idEd=:idEd, lang=:lang, slug=:slug, img=:img, tipus=:tipus, dateModified=:dateModified WHERE id=:id";
                  $stmt = $conn->prepare($sql);
