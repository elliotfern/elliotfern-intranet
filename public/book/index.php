    ...
    <script>
        $(document).ready(function(){
            function fetch_data(){
                ...
                var urlAjax = "https://elliotfern.com/controller/book.php?type=books";
                $.ajax({
                    url:urlAjax,
                    method:"POST",
                    dataType:"json",
                    success:function(data){
                        ...
                        for(var i=0; i<data.length; i++){
                            html += '<tr>';
                            html += '<td> <a id="'+data[i].id+'" title="Show book details" href="https://elliotfern.com/book/'+data[i].bookSlug+'">'+data[i].titol+'</a></td>';
                            html += '<td> <a id="'+data[i].idAutor+'" title="Show Author" href="https://elliotfern.com/author/'+data[i].autorSlug+'">'+data[i].AutNom+' '+data[i].AutCognom1+'</a></td>';
