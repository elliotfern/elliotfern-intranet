...
<script>
  ...
  function authorsTableLibrary() {
    let urlAjax = "https://" + window.location.host + "/api/biblioteca/get/autors/?type=totsAutors";
    $.ajax({
      url: urlAjax,
      method: "GET",
      dataType: "json",
      ...
      success: function(data) {
        try {
          ...
          for (let i = 0; i < data.length; i++) {
            html += '<tr>';
            html += '<td class="text-center"><a id="' + data[i].id + '" title="Author page" href="./autor/fitxa/' + data[i].slug + '"><img src="https://media.elliotfern.com/img/library-author/' + data[i].nameImg + '.jpg" style="height:70px"></a></td>';

            html += '<td><a id="' + data[i].id + '" title="Author page" href="./autor/fitxa/' + data[i].slug + '">' + data[i].AutNom + " " + data[i].AutCognom1 + '</a></td>';
