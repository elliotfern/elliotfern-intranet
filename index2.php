...
$route->add("/registre","public/auth/registre.php");
$route->add("/entrada","public/auth/login.php");

 // API SERVER 
 $route->add("/api/projects","api/projects.php");
 $route->add("/api/auth/registre","api/00_auth/registre.php");
 $route->add("/api/auth/login","api/00_auth/login.php");
 $route->add("/api/auth","api/00_auth/auth.php");
 ...
 $route->add("/api/places/{country}","api/08_biblioteca_llibres/get-library.php");
    ...
    // API VAULT
    $route->add("/api/vault/get","api/10_vault/get-vault.php");
...
if (empty($_SESSION['user']) || !session_id()) {
    ...
    exit(); 
        ...
        $route->add("/vault/elliot2/contrasenya_old","public/10_claus_acces/vault-elliot_no_utilitzar.php");
